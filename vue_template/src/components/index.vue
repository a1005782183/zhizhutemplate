...
export default {
    ...
    mounted() {
        ...
        this.search = s[s.length-1];
        // 查询所有模板
        this.$axios.get("/query_all_temp")
        .then((res)=>{
    ...
    methods: {
        ...
        get_pages: function(iter) {
            console.log('调用了'+this.search);
            this.$axios.get("/query_all_temp?page="+iter+'&s='+this.search)
            .then((res)=>{
                this.datas = res.data.data;
                this.paginate = res.data.paginate;
                // 跳转框等于当前页
